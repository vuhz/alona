...
def make(source, dest):
    ...
    """
    images = []
    base = Image_.open(source).convert('RGBA').resize(resolution)
    ...
    for i in range(frames):
        ...
        canvas = Image_.new('RGBA', size=resolution, color=(0, 0, 0, 0))
        ...
        pet = Image_.open(resource_stream(__name__, f"img/pet{i}.gif")).convert('RGBA').resize(resolution)
